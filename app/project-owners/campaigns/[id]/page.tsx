...
export default async function CampaignDetailsPage({ params: { id } }: PageProps) {
  const campaign = await getCampaign(id)
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-6">
              ...
              {campaign.monetizationPolicy ? (
                <>
                  <div>
                    <div className="flex items-center justify-between">
                      <div>
                        <h4 className="text-sm font-medium mb-2">Policy: {campaign.monetizationPolicy.name}</h4>
                        <p className="text-sm text-muted-foreground">{campaign.monetizationPolicy.description}</p>
                      </div>
                      <Link
                        href={`/project-owners/monetization?policy=${campaign.monetizationPolicy.id}`}
                  ...
                  <div>
                    ...
                    <p className="text-lg font-medium">{parseFloat(campaign.monetizationPolicy.baseRateMultiplier).toFixed(1)}x</p>
                  </div>

                  <div>
                    <h4 className="text-sm font-medium mb-2">Milestone Bonuses</h4>
                    <div className="grid gap-2">
                      {campaign.monetizationPolicy.milestoneBonus.map((milestone) => (
                  ...
                  <div>
                    ...
                    <div className="grid gap-2">
                      {campaign.monetizationPolicy.kolTierBonus.map((bonus) => (
                ...
                <div className="text-center py-6 text-muted-foreground">
                  No monetization policy applied to this campaign
                </div>
              )}
            </div>
          </CardContent>
          ...
          <CardContent className="space-y-4">
            ...
            <div>
              ...
              <div className="flex gap-2 mt-1">
                {(() => {
                  try {
                    const deliverables = campaign.requirements?.deliverables 
                      ? JSON.parse(campaign.requirements.deliverables)
                      : { list: [] };
                    return deliverables.list?.map((deliverable: string) => (
                      ...
                      <div className="flex-1">
                        ...
                        <div className="mt-4 grid grid-cols-2 gap-4">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-muted-foreground">
                              {(() => {
                                const totalApplicantImpressions = application.metrics?.reduce((sum, m) => sum + m.impressions, 0) || 0;
                                let payoutAmount = 0;

                                if (campaign.monetizationPolicy) {
                                  payoutAmount = calculatePayoutValue(
                                    campaign.cpmValue,
                                    totalApplicantImpressions,
                                    campaign.monetizationPolicy.baseRateMultiplier,
                                    campaign.monetizationPolicy.kolTierBonus.find(
                                      bonus => bonus.tier === application.creator.tier
                                    )?.bonusPercentage || '0',
                                    campaign.monetizationPolicy.milestoneBonus
                                  );
                                } else {
